...
class ConfigurationCommandsHandler(BaseHandler):
    ...
    async def handle_add_moderable_chat(self, update: EnrichedUpdate, context: CallbackContext) -> None:
        ...
        if self.config.is_chat_moderated(chat_id):
            await self.telegram_helper.send_message(context, chat_id=chat_id, text=update.locale.chat_already_moderated.format(chat_id=chat_id))
            return
        self._add_chat_to_moderable(chat_id)
        await self.telegram_helper.send_message(context, chat_id=chat_id, text=update.locale.chat_added_to_moderate.format(chat_id=chat_id))
    ...
    async def handle_remove_moderable_chat(self, update: EnrichedUpdate, context: CallbackContext) -> None:
        ...
        if not self.config.is_chat_moderated(chat_id):
            await self.telegram_helper.send_message(context, chat_id=chat_id, text=update.locale.chat_not_moderated.format(chat_id=chat_id))
            return
        self._remove_chat_from_moderable(chat_id)
        await self.telegram_helper.send_message(context, chat_id=chat_id, text=update.locale.chat_removed_from_moderate.format(chat_id=chat_id))
